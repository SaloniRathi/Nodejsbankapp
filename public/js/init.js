...
jQuery(document).ready(function(){
	// Getting selected scenario data
	let chosenScenario = getLocalStorageItem('scenario');
	chosenScenario = JSON.parse(chosenScenario)

	// Default password field state is Hidden
	let passwordDefaultState = inputFieldStates.HIDDEN;

	/* 
	* Changes the default password field state to visible only during the folliwing scenarios:
	* Behavioral Biometrics (keystrokes)
	*/
	if (chosenScenario["Flow"]["passwordHidden"] == false) {
		passwordDefaultState = inputFieldStates.VISIBLE;
	}
